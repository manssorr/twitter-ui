    ...
    <View className={`py-1.5 ${is_organization ? 'rounded-md' : 'rounded-full'}  ${className}  `}>
      {isLoading && (
        <View
          ...
          className="absolute items-center justify-center rounded-md bg-neutral-200 dark:bg-neutral-700">
...
const FollowButton = () => {
  ...
  return (
    <TouchableOpacity
      className="rounded-full bg-black px-4 py-1.5 dark:bg-white"
  ...
  return (
    <ContextMenu.Root>
      <ContextMenu.Trigger asChild>
        <TouchableOpacity
          onPress={onPress}
          className="border-b border-neutral-200 p-1 py-2 pr-4 dark:border-neutral-800">
          <View className="flex-row">
            ...
            <View className="ml-2 flex-1">
              <View className="flex-row items-center justify-between">
                <TouchableOpacity onPress={() => router.push(`/profile/${authorHandle}`)}>
                  <View className="flex-row items-center">
                    <Text className=" mr-1 text-lg font-bold text-black dark:text-white">
                      {authorName}
                    </Text>
                    {verified_badge && (
                      <Image
                        ...
                        className="h-4 w-4"
                        onError={(e) =>
                          console.log('Failed to load verified badge:', e.nativeEvent.error)
                        }
                      />
                    )}
                    <Text className="ml-1 mr-1 text-lg text-neutral-500 dark:text-neutral-400">
                      @{authorHandle}
                    </Text>
                    <Text className="text-lg text-neutral-500 dark:text-neutral-400">
                      {' '}
                      · {formatTime(itemData.posted_time, false)}
                    </Text>
                  </View>
                </TouchableOpacity>
                ...
                {!detailView && (
                  <DropdownMenu.Root>
                    <DropdownMenu.Trigger>
                      <TouchableOpacity className="-m-1 p-1 opacity-70">
                        <Feather
                        ...
                        <DropdownMenu.SubContent>
                          <DropdownMenu.Item key="add-remove">
                            <DropdownMenu.ItemTitle>
                              Add/remove @{authorHandle} from list
                            </DropdownMenu.ItemTitle>
              ...
              <Text className="mt-1 text-lg leading-6 text-black dark:text-white">
                {itemData.message}
              </Text>
              {itemData.media_url && (
                <Galeria urls={[itemData.media_url].filter((url) => !!url) as string[]}>
                  <Galeria.Image>
                    <Image
                      ...
                      className="mt-2 aspect-[4/4] w-full rounded-lg"
                      contentFit="cover"
                    />
                  </Galeria.Image>
                </Galeria>
      ...
      <ContextMenu.Content>
        ...
        <ContextMenu.Item
          key="report"
          destructive
          onSelect={() => console.log('Report post', itemData.contentId)}>
        ...
        <ContextMenu.Item
          key="community-note"
          onSelect={() => console.log('Request Community Note', itemData.contentId)}>
        ...
        <ContextMenu.Item
          key="follow"
          onSelect={() => console.log('Follow/Unfollow', userInfo?.id)}>
        ...
        <ContextMenu.Sub>
          ...
          <ContextMenu.SubContent>
            <ContextMenu.Item
              key="interactions"
              onSelect={() => console.log('View Interactions', itemData.contentId)}>
            ...
            <ContextMenu.Item
              key="add-to-lists"
              onSelect={() => console.log('Add to Lists', userInfo?.id)}>
            ...
            <ContextMenu.Item
              key="block"
              destructive
              onSelect={() => console.log('Block User', userInfo?.id)}>
    ...
    <View className="px-4 dark:border-neutral-800 ">
        ...
        <View className="flex-row items-center">
          ...
          <TouchableOpacity onPress={() => router.push(`/profile/${authorHandle}`)}>
            <View className="flex-col">
              ...
              <Text className="ml-1 text-lg text-neutral-500  dark:text-neutral-400">
                @{authorHandle}
              </Text>
            </View>
          </TouchableOpacity>
      ...
      <View className="mt-2">
        ...
        {itemData.media_url && (
          <Galeria urls={[itemData.media_url].filter((url) => !!url) as string[]}>
            <Galeria.Image>
              <Image
                source={{ uri: itemData.media_url }}
                className="mt-4 aspect-[4/4] w-full rounded-lg"
                contentFit="cover"
              />
            </Galeria.Image>
          </Galeria>
        ...
        <View className="mt-2 flex-row items-center gap-2">
          <Text className="mt-2 text-lg text-neutral-500 dark:text-neutral-400">
            {formatTime(itemData.posted_time, true)}
          </Text>
          <Text className="mt-2 text-lg text-neutral-500 dark:text-neutral-400">
            <Text className="font-semibold text-black dark:text-white">{itemData.view_count}</Text>{' '}
            Views
          </Text>
        </View>

        <View className="">
          <EngagementActions itemData={itemData} detailView />
        </View>
      </View>
    </View>
  );
