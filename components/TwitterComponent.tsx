...
const TopNavigationBar: React.FC<TopNavigationBarProps> = ({
  ...
  const { calculatedCenterWidth, minimumSideWidth } = useMemo(() => {
    const centerW = centerSectionExists
      ? MIN_CENTER_SECTION_WIDTH_PERCENTAGE * dimensions.width
      : 0;
...
type ScrollingListWithHeaderProps<ItemT, SectionT = DefaultSectionT> = Omit<
...
>(
  ...
  const navBarVisibility = useDerivedValue(() => {
    ...
    return interpolate(
      scrollOffset.value,
      [
        expandedHeaderHeight.value * (expandedHeaderCollapseThreshold - 0.01),
        expandedHeaderHeight.value * expandedHeaderCollapseThreshold,
      ],
      [0, 1],
      Extrapolate.CLAMP
    );
